...
export const keyboardBtns = document.querySelectorAll(".keyboard");
let currentCell;
let column = 0;
...
export const activeKeyboardFinger = (e) => {
...
export const checkWord = () => {
    ...
    const clearBoard = (nodes, nodeIndex) => {
        if (nodeIndex < nodes.length) {
            nodes[nodeIndex].classList.add("check");
            nodes[nodeIndex].classList.remove("green", "yellow", "grey");
            if (nodes[nodeIndex].classList.contains("cell"))
                nodes[nodeIndex].innerHTML = "";
            setTimeout(() => {
                if (nodes[nodeIndex])
                    nodes[nodeIndex].classList.remove("green", "yellow", "grey");
            }, 500);
    ...
    const resetGame = (resetBtn, clearBoard) => {
    ...
    if (gameStatus) {
        ...
        if (marcActive == true)
            setTimeout(() => resetGame(resetBtn, clearBoard), 2500);
